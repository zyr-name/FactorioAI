-- All simulation state lives in storage, including leases. Do not mutate it
-- from on_load: that event also runs when a multiplayer client joins.
local LEASE_TICKS = 180
local STUCK_TICKS = 120
local MAX_DISTANCE = 64
local ARRIVAL_DISTANCE = 0.3
...
local function initialize()
  storage.bridge = storage.bridge or {schema = 1}
...
script.on_load(function()
  -- Storage may contain a live lease from the saved tick. Defer mutation until
  -- the first event with game access, then stop before entity updates continue.
  just_loaded = true
end)
...
local function status()
  ...
  local entity = valid_character()
  local result = {
    tick = game.tick, protocol = 1, lease_ticks = LEASE_TICKS,
...
local function spawn(request)
  ...
  if bridge.companion then
    ...
    return status() -- Repeated spawn/connect never creates a second character.
  end
...
local function dispatch(request)
  ...
  if request.action == "move" then
    local companion = storage.bridge.companion
    if companion.motion and companion.motion.command_id == request.id then return status() end
    if not finite_number(request.x) or not finite_number(request.y) then
      fail("invalid_target", "Provide finite x and y coordinates.")
    end
    local position = companion.entity.position
    local distance = math.sqrt((request.x - position.x)^2 + (request.y - position.y)^2)
    if distance > MAX_DISTANCE then fail("target_too_far", "Move at most 64 tiles per command.") end
    halt("superseded")
    companion.motion = {
      command_id = request.id, state = "moving", target = {x = request.x, y = request.y},
...
commands.add_command("companion", "Factorio AI companion JSON API (RCON only).", function(event)
  ...
  if not ok then
    if type(value) ~= "table" then
      -- Unexpected faults stop actions instead of leaving an active walker.
      halt("bridge_error")
      storage.bridge.session = nil
...
script.on_event(defines.events.on_tick, function()
  ...
  if dx * dx + dy * dy <= ARRIVAL_DISTANCE^2 then halt("arrived"); return end
  if game.tick >= motion.deadline_tick then halt("timed_out"); return end
  local progress = motion.progress_position
  if (position.x - progress.x)^2 + (position.y - progress.y)^2 > 0.01 then
    motion.progress_position = {x = position.x, y = position.y}
    motion.progress_tick = game.tick
  elseif game.tick - motion.progress_tick >= STUCK_TICKS then
    halt("blocked"); return
  end
  -- Direct steering at normal character speed; collision rules remain in force.
  local horizontal = math.abs(dx) > 0.2 and (dx > 0 and 1 or -1) or 0
  local vertical = math.abs(dy) > 0.2 and (dy > 0 and 1 or -1) or 0
  local directions = {
